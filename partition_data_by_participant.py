...
def load_npz_data(eeg_dir, eye_dir, file_name, cv_number):
    ...
    # 3 sessions, 15 videos per session (3 videos invoking each of the 5 emotions)
    ...
    # there will be hundreds of samples for each emotion
...
if __name__ == '__main__':
    ...
    for p_num in range(1, len(file_list)+1):
        for emotion in emotion_dict.values():
            create_if_not_exists(os.path.join(partition_dir, 'support', emotion, f"p_{p_num}"))
            create_if_not_exists(os.path.join(partition_dir, 'query', emotion, f"p_{p_num}"))
    ...
    for file_name in file_list:
        ...
        for sample in train_data: # save train/support data to their corresponding emotion folders
            ...
            label = int(sample[-1])
            save_dir = os.path.join(partition_dir, 'support', emotion_dict[label], f"p_{num}")
            # concatenate with the filename
            # save the label to where it belongs to            
            save_file = os.path.join(save_dir, f"{emotion_count[label]}")
        ...
        for sample in test_data: # do the same thing for the test/query data
            label = sample[-1]
            save_dir = os.path.join(partition_dir, 'query', emotion_dict[label], f"p_{num}")
            save_file = os.path.join(save_dir, f"{emotion_count[label]}")
            emotion_count[label] += 1
            np.save(save_file, sample)
